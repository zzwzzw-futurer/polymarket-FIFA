...
These settings are also recorded in `edgeone.json`, so importing the GitHub repository into EdgeOne Pages can reuse the same build configuration.
...
For GitHub Actions deployment, add a repository secret named `EDGEONE_API_TOKEN`. The `Deploy to EdgeOne Pages` workflow builds `dist` and runs:
